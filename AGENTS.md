...
- Diese Integrationstests brauchen die Wegwerf-DB aus `tests/docker-compose.test.yml`: `npm.cmd run test:php:db:up` (Port 3307, tmpfs, Schema aus `server/db/schema.mysql.sql`). Ohne laufenden Container **überspringen** sie sich, die Suite bleibt grün - vor einem Commit an der API also hochfahren, sonst prüft niemand die Handler.
